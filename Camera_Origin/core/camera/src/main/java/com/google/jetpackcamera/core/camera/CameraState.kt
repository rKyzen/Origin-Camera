...
data class CameraState(
    ...
    val mfsState: MfsState = MfsState.Idle
)
...
/**
 * Represents the state of multi-frame stacking (MFS) capture.
 */
sealed interface MfsState {
    /** No MFS capture is in progress. */
    data object Idle : MfsState

    /** MFS is currently capturing burst frames. */
    data class Capturing(val capturedFrames: Int, val totalFrames: Int) : MfsState

    /** MFS is merging captured frames. */
    data object Merging : MfsState

    /** MFS is saving the final result. */
    data object Saving : MfsState

    /** MFS capture completed successfully. */
    data object Saved : MfsState

    /** MFS capture failed. */
    data class Failed(val errorMessage: String) : MfsState
}
