...
fun captureUiState(
    ...
    return combine(
        ...
        CaptureUiState.Ready(
            ...
            mfsProgressUiState = cameraState.mfsState.toMfsProgressUiState()
        )
    }
}

/**
 * Converts [CameraState.MfsState] to [MfsProgressUiState] for the UI layer.
 */
internal fun MfsState.toMfsProgressUiState(): MfsProgressUiState = when (this) {
    is MfsState.Idle -> MfsProgressUiState.Hidden
    is MfsState.Capturing -> MfsProgressUiState.Capturing(capturedFrames, totalFrames)
    is MfsState.Merging -> MfsProgressUiState.Processing
    is MfsState.Saving -> MfsProgressUiState.Saving
    is MfsState.Saved -> MfsProgressUiState.Saved
    is MfsState.Failed -> MfsProgressUiState.Failed(errorMessage)
}
